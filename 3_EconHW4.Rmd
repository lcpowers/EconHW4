...
A_vector <- seq(0,220,length.out=221)
...
```{r}
#C_nls <- nls(MC_C ~ (Y + Q*Abatement + Q*Abatement^2))
C_demand <- nls(MB_C ~ (260.243550*(exp(a*Abatement))),
                data = C_df_nls,
                start = list(a=.01),
                trace = TRUE)
summary(C_demand)

C_df_nls$demand <- predict(C_demand, list(Abatement = C_vector))

C_df_demand <- data.frame(C_vector,C_lm_demand)
colnames(C_df_demand) <- c("Abatement","Demand")

ggplot(data=C_df_nls, aes(x=Abatement, y=MB_C))+
  geom_line(aes(y=demand))+
  geom_line()+
  geom_line(data=C_df_nls, aes(y=MC_C))+
  scale_x_continuous(expand = c(0, 0))+
  scale_y_continuous(expand = c(0, 0))

write.csv(C_df_nls, file = "MyData.csv")

#y = (0,265)
#x = (220,0)
C_demand_slope <- (265-0)/(0-220)

C_df_nls$demand <- 265 + C_demand_slope*C_df_nls$Abatement
```
...
#C Supply
#P(Q) ~ 260.243550*0.9881161^(Q)
#Q(P) = (log(P)-log(260))/log(.988)

#Ag demand function
b_demand$coefficients

Ag_Dfun <- function(P){
  A_D <- ifelse((95.1870630-P)/(0.5768419)<0,0,(95.1870630-P)/(0.5768419))
  B_D <- ifelse((49.0665582-Price)/(0.1987443)<0,0,(49.0665582-Price)/(0.1987443))
  C_D <- (log(P)-log(260.243550))/log(0.9881161)
  Ag_Q <- A_D + B_D + C_D
  return(Ag_Q)
}

Price <- seq(0,260,length.out=261)
ag_Demand <- Ag_Dfun(Price)
aggregate <- data.frame(Price,ag_Demand)

ggplot(aggregate, aes(x=ag_Demand,y=Price))+
  geom_line()+
  scale_x_continuous(expand = c(0, 0))+
  scale_y_continuous(expand = c(0, 0))
```
